...
export default function App() {
  ...
  useEffect(() => {
    axios.get('https://student-store-app-108b2e4e9abd.herokuapp.com/store').then(response => {
  ...
  */
  ...
  let handleAddItemToCart = (productId) => {
    ...
    let cartCopy = shoppingCart.filter(current => {
      if (current.itemId === productId) {
  ...
  let handleOnSubmitCheckoutForm = async () => {
    try {
      let response = await axios.post("https://student-store-app-108b2e4e9abd.herokuapp.com/store", {
        shoppingCart,
        user: {
  ...
  let handleOnPastOrders = () => {
    axios.get('https://student-store-app-108b2e4e9abd.herokuapp.com/store/purchases').then(response => {
      console.log(response)
